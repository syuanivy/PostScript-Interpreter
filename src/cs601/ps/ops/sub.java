...
public class sub extends PSOperator {
	@Override
	public void execute(PSInterpreter interpreter) {
		...
		if ( left instanceof PSInt && right instanceof PSInt ) {
			int result = (int)(left.floatValue() - right.floatValue());
			interpreter.push(new PSInt(result));
		}
		else { // one or both are float
			float result = (left.floatValue() - right.floatValue());
